    ...
    const fetchCourseData = async () => {
      ...
      try {
        ...
        if (token) {
          ...
          setIsEnrolled(progressResponse.data.isEnrolled || progress > 0);
        }
      } catch (err) {
          ...
          <DetailCard elevation={0} level={course.level}>
            {course.imageUrl && (
              <Box
                component='img'
                src={course.imageUrl}
                alt={`Image du cours ${course.titre}`}
                sx={{ width: '100%', height: 250, objectFit: 'cover', borderRadius: '16px', mb: 3 }}
              />
            )}
            {course.isCertifying && (
            ...
            <Stack direction='row' spacing={2} sx={{ mb: 3 }}>
              <Chip
                ...
                sx={{ backgroundColor: `${colors.navy}33`, color: '#ffffff', fontWeight: 500 }}
              />
              <Chip
            ...
            <Typography
              sx={{ color: 'rgba(255, 255, 255, 0.8)', fontSize: '1.2rem', lineHeight: 1.6, mb: 4 }}
            >
            ...
            <Button
              ...
              sx={{
                ...
                '&:disabled': { background: 'grey', boxShadow: 'none' },
              }}
              endIcon={<ArrowRight size={24} />}
              aria-label={isEnrolled ? 'Continuer le cours' : "S'inscrire au cours"}
            >
