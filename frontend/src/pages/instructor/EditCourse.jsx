...
const EditCourse = () => {
    ...
    const fetchCourse = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/instructor/${user.id}/courses/${courseId}`,
        ...
        setFormData({
          ...
          domaineId: response.data.data.domaineId || '',
          niveau: response.data.data.niveau || 'Alfa',
          contenu: response.data.data.contenu || '',
          quizzes: response.data.data.quizzes || [],
          statutApprobation: response.data.data.statutApprobation || 'PENDING',
          estPublie: response.data.data.estPublie || false,
        });
      } catch (err) {
        console.error('Erreur lors de la récupération du cours:', err);
        setError(err.response?.data?.message || 'Erreur lors de la récupération du cours. Veuillez réessayer.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Erreur lors de la mise à jour du cours. Veuillez réessayer.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <FormContainer>
        ...
        <Container maxWidth={false} disableGutters>
          <Box sx={{ p: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
            <EditIcon sx={{ fontSize: 40, color: colors.fuschia || '#f13544' }} />
          ...
          <FormCard>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <StyledButton type="submit" disabled={isLoading} fullWidth sx={{ mt: 2 }}>
                {isLoading ? <CircularProgress size={24} sx={{ color: colors.white || '#ffffff' }} /> : 'Sauvegarder le Cours'}
              </StyledButton>
            </form>
          </FormCard>
        </Container>
